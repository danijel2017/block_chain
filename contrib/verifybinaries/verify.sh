#!/usr/bin/env bash
# Copyright (c) 2016 The Grailum Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

###   This script attempts to download the signature file SHA256SUMS.asc from
###   grailumcore.org and grailum.org and compares them.
...
WORKINGDIR="/tmp/grailum_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://grailumcore.org"
HOST2="https://grailum.org"
BASEDIR="/bin/"
VERSIONPREFIX="grailum-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'grailum-',
...
if [ $? -ne 0 ]; then
   echo "grailum.org failed to provide signature file, but grailumcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "grailum.org and grailumcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Grailum Core binary release signing key installed?"
   fi
