...
const char * const GRAILUM_CONF_FILENAME = "grailum.conf";
const char * const GRAILUM_PID_FILENAME = "grailumd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "grailumd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Grailum
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Grailum
    // Mac: ~/Library/Application Support/Grailum
    // Unix: ~/.grailum
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Grailum";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Grailum";
#else
    // Unix
    return pathRet / ".grailum";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", GRAILUM_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", GRAILUM_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Grailum Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Grailum Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Grailum Core developers";
    }
