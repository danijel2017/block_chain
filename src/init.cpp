 ...
 * between grailumd, and grailum-qt and non-server tools.
*/
...
#ifndef WIN32
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", GRAILUM_PID_FILENAME), false, OptionsCategory::OPTIONS);
#else
...
/** Sanity checks
 *  Ensure that Grailum is running in a usable environment with all
...
{
    // Make sure only a single Grailum process is using the data directory.
...
bool AppInitMain()
{
    ...
    fs::path config_file_path = GetConfigFile(gArgs.GetArg("-conf", GRAILUM_CONF_FILENAME));
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if grailum is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if grailum is started while in a temporary directory.\n",
