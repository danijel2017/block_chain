...
static void SetupGrailumTxArgs()
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " grailum-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  grailum-tx [options] <hex-tx> [commands]  Update hex-encoded grailum transaction\n" +
            "or:     grailum-tx [options] -create [commands]   Create hex-encoded grailum transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded grailum transaction
